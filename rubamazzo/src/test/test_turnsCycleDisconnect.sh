...
CATIA_CARD0=$(echo "$GAME_STATE" | jq -r '.playerHands.Catia[0]')
...
CATIA_CARD0_ENCODED=$(echo "$CATIA_CARD0" | sed 's/ /%20/g')
MIRKO_CARD_ENCODED=$(echo "$MIRKO_CARD" | sed 's/ /%20/g')
SARA_CARD_ENCODED=$(echo "$SARA_CARD" | sed 's/ /%20/g')

echo "Catia ha la carta: $CATIA_CARD0"
echo "Mirko ha la carta: $MIRKO_CARD"
echo "Sara ha la carta: $SARA_CARD"


echo "### Catia fa una mossa con la carta: $CATIA_CARD0 ###"
curl -X POST "$BASE_URL/makeMove/$GAME_ID?playerName=Catia&move=$CATIA_CARD0_ENCODED"
echo -e "\n-----------------------------\n"
...
CATIA_CARD1=$(echo "$GAME_STATE" | jq -r '.playerHands.Catia[1]')
# Codifica le carte per l'URL
CATIA_CARD1_ENCODED=$(echo "$CATIA_CARD1" | sed 's/ /%20/g')
echo "Catia ha la carta: $CATIA_CARD1"

echo "### Disconnessione di Catia (di turno) ###"
...
echo "### Catia fa una mossa con la carta: $CATIA_CARD1 ###"
curl -X POST "$BASE_URL/makeMove/$GAME_ID?playerName=Catia&move=$CATIA_CARD1_ENCODED"
echo -e "\n-----------------------------\n"
echo "### Verifica se i turni sono stati aggiornati correttamente ###"
curl -X GET "$BASE_URL/gameState/$GAME_ID"
echo -e "\n-----------------------------\n"
